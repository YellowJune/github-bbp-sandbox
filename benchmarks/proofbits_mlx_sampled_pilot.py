...
def make_pilot_kernel():
 return mx.fast.metal_kernel(
     name='pb_sampled_exact_pilots_v2',
     input_names=['high','low','hidden','indices'],
     output_names=['scores'],source=PILOT_BATCH_SRC)


def make_indices(V,K):
 # Uniform coverage is only a bound-quality heuristic. Exactness does not depend on it.
 return ((mx.arange(K,dtype=mx.uint32)*V)//K).astype(mx.uint32)


def sampled_decision(base_ks,pilot_k,high,low,h,V,idx):
 _,upper_k,_,refine_k=base_ks
 K=int(idx.shape[0])
 # U and exact pilot scores can be formed without the old U->argmax(U)->pilot chain.
 U=upper_k(inputs=[high,h],grid=(V*32,1,1),threadgroup=(32,1,1),
           output_shapes=[(V,)],output_dtypes=[mx.float32],init_value=0.0)[0]
 ps=pilot_k(inputs=[high,low,h,idx],grid=(K*32,1,1),threadgroup=(32,1,1),
            output_shapes=[(K,)],output_dtypes=[mx.float32],init_value=-3.402823466e38)[0]
 B=mx.reshape(mx.max(ps),(1,))
 E=refine_k(inputs=[high,low,h,U,B],grid=(V*32,1,1),threadgroup=(32,1,1),
            output_shapes=[(V,)],output_dtypes=[mx.float32],init_value=-3.402823466e38)[0]
 return mx.argmax(E).astype(mx.uint32)


def sampled_diagnostic(base_ks,pilot_k,high,low,h,V,idx):
 # Untimed diagnostic only: survivor count is intentionally outside serving timing.
 _,upper_k,_,_=base_ks
 K=int(idx.shape[0])
 U=upper_k(inputs=[high,h],grid=(V*32,1,1),threadgroup=(32,1,1),
           output_shapes=[(V,)],output_dtypes=[mx.float32],init_value=0.0)[0]
 ps=pilot_k(inputs=[high,low,h,idx],grid=(K*32,1,1),threadgroup=(32,1,1),
...
def run(model,tok,prompt,mode,base_ks,pilot_k,w16,high,low,idxs,n):
 ...
 for _ in range(n):
  ...
  if mode=='dense': y=base.call_dense(base_ks[0],w16,h,V)
   ...
   K=int(mode[6:]); y=sampled_decision(base_ks,pilot_k,high,low,h,V,idxs[K])
...
def one_hidden(model,tok,prompt,D):
 kv=cache_mod.make_prompt_cache(model)
 b=model.model(ids(tok,prompt)[None],cache=kv)
 h=b[:,-1,:].reshape((D,)).astype(mx.float32)
 mx.eval(h,[c.state for c in kv])
...
def main():
 ...
 model.set_dtype(mx.float16); mx.eval(model.parameters()); base.MODEL=MODEL
 base_ks=base.make_kernels(); pilot_k=make_pilot_kernel()
 w16,high,low=base.prepare_weights(model); V,D=[int(x) for x in w16.shape]
 idxs={K:make_indices(V,K) for K in PILOT_KS}; mx.eval(*idxs.values())
 ...
 for pi,p in enumerate(PROMPTS):
  ...
  for m in order:
   ...
   res[m]=run(model,tok,p,m,base_ks,pilot_k,w16,high,low,idxs,TOKENS)
  d,c,nat=res['dense'],res['current'],res['native']
  r={'prompt_index':pi,'order':order,'current_exact':d['tokens']==c['tokens'],
     'dense_ms':d['median_ms'],'current_ms':c['median_ms'],'native_ms':nat['median_ms']}
  for K in PILOT_KS:
   q=res[f'sample{K}']
   r[f'k{K}_exact']=q['tokens']==d['tokens']
   r[f'k{K}_ms']=q['median_ms']
   r[f'k{K}_vs_current']=c['median_ms']/q['median_ms']
   r[f'native_over_k{K}']=nat['median_ms']/q['median_ms']
  rows.append(r)

 # One hidden-state survivor diagnostic per prompt and K, explicitly untimed.
 diagnostics=[]
 for pi,p in enumerate(PROMPTS):
  h=one_hidden(model,tok,p,D)
  entry={'prompt_index':pi}
  for K in PILOT_KS:
   entry[f'k{K}_survivors']=sampled_diagnostic(base_ks,pilot_k,high,low,h,V,idxs[K])
  diagnostics.append(entry)

 out={'kind':'proofbits_fp16_sampled_exact_pilot_fair_v2','model':MODEL,
      'pilot_Ks':PILOT_KS,'n_prompts':len(PROMPTS),'tokens_per_prompt':TOKENS,
      'rows':rows,'untimed_diagnostics':diagnostics}
 for K in PILOT_KS:
  out[f'k{K}_all_exact']=all(r[f'k{K}_exact'] for r in rows)
  out[f'k{K}_median_vs_current']=med([r[f'k{K}_vs_current'] for r in rows])
  out[f'k{K}_mean_vs_current']=float(statistics.mean(r[f'k{K}_vs_current'] for r in rows))
  ...
  out[f'k{K}_diagnostic_median_survivors']=med([x[f'k{K}_survivors'] for x in diagnostics])
 out['note']='Fair timing: no survivor-count scan/evaluation occurs in sampled serving paths. Diagnostics are separate after all timed runs. Any exact pilot score is a valid global lower bound, so correctness is independent of the pilot heuristic.'
 Path('experiments/artifacts').mkdir(parents=True,exist_ok=True)
 Path('experiments/artifacts/proofbits_mlx_sampled_pilot.json').write_text(json.dumps(out,indent=2))
