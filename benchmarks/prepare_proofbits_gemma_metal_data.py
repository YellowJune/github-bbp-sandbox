...
MODEL='unsloth/gemma-3-270m';N=6;MAX_LEN=96
OUT=Path('benchmarks/tmp_proofbits_gemma');OUT.mkdir(parents=True,exist_ok=True)
torch.set_grad_enabled(False)
tok=AutoTokenizer.from_pretrained(MODEL);m=AutoModelForCausalLM.from_pretrained(MODEL,torch_dtype=torch.float32,low_cpu_mem_usage=True);m.eval()
...
high.numpy().tofile(OUT/'high_u8.bin');low.numpy().tofile(OUT/'low_u8.bin')
