...
MASS={'B':10.81,'Al':26.9815385,'Sc':44.95591,'Ti':47.867,'CuA':63.546,'CuB':63.546,'N':14.007}
BASE={'B':'B','Al':'Al','Sc':'Sc','Ti':'Ti','CuA':'Cu','CuB':'Cu','N':'N'}
...
def labels(s):
    ...
    for site in s:
        ...
        if e!='Cu':z.append(e);continue
        fx,fy,_=site.frac_coords;ix=int(round((fx%1)*3))%3;iy=int(round((fy%1)*2))%2
        z.append('CuA' if (ix+iy)%2==0 else 'CuB')
    return z
...
def write_scf(path,prefix,s,labs,pm,ppdir,outdir,U):
    ...
    a,b,c=s.lattice.abc;k=(max(2,min(4,round(18/a))),max(2,min(5,round(18/b))),max(5,min(10,round(24/c))))
    ...
    for i,x in enumerate(sp,1):
        mag=.58 if x=='CuA' else (-.58 if x=='CuB' else (.03 if x=='N' else 0.0));L.append(f' starting_magnetization({i})={mag:.3f},')
    L+=['/','&ELECTRONS',' conv_thr=1.d-8, mixing_beta=.20, electron_maxstep=360,','/']
...
def total_dos(prefix):
    f=prefix+'.pdos_tot'
    if not os.path.exists(f):return None,None
    a=np.atleast_2d(np.loadtxt(f,comments='#'));return a[:,0],a[:,1:].sum(1)

def metrics(prefix,ef):
    E,cu,ncu=sum_orb(prefix,['CuA','CuB'],'d');EN,n,nn=sum_orb(prefix,['N'],'p');Et,td=total_dos(prefix)
    if E is None or n is None or not np.isfinite(ef):return {}
    tr=np.trapezoid
    def integ(y,lo,hi):
    ...
    cd,lp=cent(cu),cent(n)
    rec={'cu_d_pm0p10':integ(cu,-.10,.10),'n_p_pm0p10':integ(n,-.10,.10),'cu_d_pm0p25':integ(cu,-.25,.25),'n_p_pm0p25':integ(n,-.25,.25),'pd_hybrid_pm0p25':integ(np.sqrt(np.clip(cu,0,None)*np.clip(n,0,None)),-.25,.25),'pd_hybrid_pm0p50':integ(np.sqrt(np.clip(cu,0,None)*np.clip(n,0,None)),-.50,.50),'cu_d_centroid_eV':cd,'n_p_centroid_eV':lp,'pd_centroid_sep_eV':abs(cd-lp),'nCuD_channels':ncu,'nNp_channels':nn}
    if Et is not None:
        m=(Et>=ef-.10)&(Et<=ef+.10);rec['total_DOS_pm0p10']=float(np.trapezoid(td[m],Et[m])) if m.sum()>=2 else np.nan;rec['DOS_at_EF_interp']=float(np.interp(ef,Et,td))
    return rec

def main():
    ap=argparse.ArgumentParser();ap.add_argument('--cif',required=True);ap.add_argument('--pressure-atm',type=float,required=True);ap.add_argument('--winner',required=True);ap.add_argument('--ppdir',required=True);ap.add_argument('--meta',required=True);ap.add_argument('--scratch',required=True);ap.add_argument('--out',required=True);ap.add_argument('--pw',default='/opt/espresso/7.5/pw.x');ap.add_argument('--proj',default='/opt/espresso/7.5/projwfc.x');a=ap.parse_args()
    ...
    for U in [0.,6.]:
        tag=f'U{int(U)}';od=scratch/tag;od.mkdir(exist_ok=True);pref=f'{a.winner}_{int(a.pressure_atm)}atm_{tag}'
        inp=out/f'scf_{tag}.in';oo=out/f'scf_{tag}.out';write_scf(inp,pref,s,labs,pm,a.ppdir,str(od),U);rc=run(a.pw,str(inp),str(oo));rec={'winner':a.winner,'pressure_atm':a.pressure_atm,'U_Cu_eV':U,'scf_rc':rc,'natoms':len(s)}
        if rc==0:
            txt=oo.read_text(errors='ignore');ef=num(r'the Fermi energy is\s+([-0-9.Ee+]+)',txt);rec.update({'fermi_eV':ef,'energy_Ry':num(r'!\s+total energy\s+=\s+([-0-9.Ee+]+)\s+Ry',txt),'total_magnetization':num(r'total magnetization\s+=\s+([-0-9.Ee+]+)',txt),'absolute_magnetization':num(r'absolute magnetization\s+=\s+([-0-9.Ee+]+)',txt)})
