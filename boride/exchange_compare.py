...
MASS={'Sc':44.95591,'Ti':47.867,'Ca':40.078,'CuA':63.546,'CuB':63.546,'B':10.81,'O':15.999}
BASE={'Sc':'Sc','Ti':'Ti','Ca':'Ca','CuA':'Cu','CuB':'Cu','B':'B','O':'O'}
...
def control():
    ...
    for y in range(2):
        for x in range(3):
            sy += ['Ca','Cu','O','O'];fr += [[(x+.5)/3,(y+.5)/2,.5],[x/3,y/2,0],[(x+.5)/3,y/2,0],[x/3,(y+.5)/2,0]]
...
def write_input(path,prefix,s,labs,pm,ppdir,outdir,mode):
    ...
    L=['&CONTROL'," calculation='scf',",f" prefix='{prefix}',",f" pseudo_dir='{ppdir}',",f" outdir='{outdir}',"," tstress=.true., tprnfor=.true., disk_io='low',",'/','&SYSTEM',' ibrav=0,',f' nat={len(s)}, ntyp={len(sp)},',' ecutwfc=60.0, ecutrho=480.0,'," occupations='smearing', smearing='mv', degauss=.012,",' nspin=2,']
    for i,x in enumerate(sp,1):
        if x=='CuA':mag=.65
        elif x=='CuB':mag=.65 if mode=='FM' else -.65
        elif x in {'B','O'}:mag=.02
        else:mag=0.0
    ...
    L += [f' {x} {MASS[x]:.7f} {pm[x]}' for x in sp]+['ATOMIC_POSITIONS crystal']
...
def calc(label,s,ppdir,meta,scratch,out,pw):
    ...
    for mode in ['AFM','FM']:
        ...
        inp=Path(out)/f'{label}_{mode}.in';oo=Path(out)/f'{label}_{mode}.out';write_input(inp,f'{label}_{mode}',s,labs,pm,ppdir,str(od),mode)
        rc=run(pw,str(inp),str(oo));rec={'rc':rc}
        if rc==0:
            txt=oo.read_text(errors='ignore');rec.update({'energy_Ry':last(r'!\s+total energy\s+=\s+([-0-9.Ee+]+)\s+Ry',txt),'total_magnetization':last(r'total magnetization\s+=\s+([-0-9.Ee+]+)',txt),'absolute_magnetization':last(r'absolute magnetization\s+=\s+([-0-9.Ee+]+)',txt)})
        ans[mode]=rec
    return ans

def main():
    ap=argparse.ArgumentParser();ap.add_argument('--cif',required=True);ap.add_argument('--engine',required=True);ap.add_argument('--ppdir',required=True);ap.add_argument('--meta',required=True);ap.add_argument('--scratch',required=True);ap.add_argument('--out',required=True);ap.add_argument('--pw',default='/opt/espresso/7.5/pw.x');a=ap.parse_args()
    ...
    res={'candidate':'ScTi5Cu6B12','engine_geometry':a.engine,'control':cr,'candidate_states':rr}
    ok=all(x.get('rc')==0 for x in [cr['AFM'],cr['FM'],rr['AFM'],rr['FM']])
    if ok:
        dc=(cr['FM']['energy_Ry']-cr['AFM']['energy_Ry'])*RY_TO_EV;dr=(rr['FM']['energy_Ry']-rr['AFM']['energy_Ry'])*RY_TO_EV
        ratio=dr/dc if abs(dc)>1e-8 else np.nan;J=.130*ratio;Teq=133.5*ratio
        res.update({'deltaE_control_FMminusAFM_eV':dc,'deltaE_candidate_FMminusAFM_eV':dr,'exchange_ratio_to_CaCuO2':ratio,'J_calibrated_eV':J,'Tc_scale_if_parent_efficiency_K':Teq,'passes_300K_exchange_scale':bool(np.isfinite(Teq) and Teq>=300),'passes_strong_80pct_efficiency_gate':bool(np.isfinite(J) and J>=.3652),'passes_very_strong_70pct_efficiency_gate':bool(np.isfinite(J) and J>=.4173),'magnetic_state_sanity':bool(abs(cr['AFM'].get('total_magnetization',99))<2.0 and abs(rr['AFM'].get('total_magnetization',99))<2.5 and rr['FM'].get('absolute_magnetization',0)>0.5*rr['AFM'].get('absolute_magnetization',0))})
