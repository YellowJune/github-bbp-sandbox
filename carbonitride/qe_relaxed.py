...
MASS={'Ti':47.867,'Nb':92.90637,'CuA':63.546,'CuB':63.546,'C':12.011,'N':14.007}
BASE={'Ti':'Ti','Nb':'Nb','CuA':'Cu','CuB':'Cu','C':'C','N':'N'}
...
def write_scf(path,prefix,s,labs,pm,ppdir,outdir,U):
    ...
    a,b,c=s.lattice.abc;k=(max(2,min(4,round(18/a))),max(2,min(5,round(18/b))),max(4,min(9,round(22/c))))
    L=['&CONTROL'," calculation='scf',",f" prefix='{prefix}',",f" pseudo_dir='{ppdir}',",f" outdir='{outdir}',"," tstress=.true., tprnfor=.true., disk_io='low',",'/','&SYSTEM',' ibrav=0,',f' nat={len(s)}, ntyp={len(sp)},',' ecutwfc=60.0, ecutrho=480.0,'," occupations='smearing', smearing='mv', degauss=.018,",' nspin=2,']
    for i,x in enumerate(sp,1):
        mag=.55 if x=='CuA' else (-.55 if x=='CuB' else (.03 if x in {'C','N'} else 0.0));L.append(f' starting_magnetization({i})={mag:.3f},')
    L+=['/','&ELECTRONS',' conv_thr=1.d-8, mixing_beta=.18, electron_maxstep=400,','/']
...
def metrics(prefix,ef):
    E,cu,ncu=sumorb(prefix,['CuA','CuB'],'d');EC,c,nc=sumorb(prefix,['C'],'p');EN,n,nn=sumorb(prefix,['N'],'p')
    if E is None or not np.isfinite(ef):return {}
    lig=np.zeros_like(cu)
    if c is not None and len(c)==len(E):lig+=c
    ...
    return {'cu_d_pm0p10':integ(cu,-.10,.10),'ligand_p_pm0p10':integ(lig,-.10,.10),'cu_d_pm0p25':integ(cu,-.25,.25),'ligand_p_pm0p25':integ(lig,-.25,.25),'pd_hybrid_pm0p25':integ(np.sqrt(np.clip(cu,0,None)*np.clip(lig,0,None)),-.25,.25),'pd_hybrid_pm0p50':integ(np.sqrt(np.clip(cu,0,None)*np.clip(lig,0,None)),-.50,.50),'cu_d_centroid_eV':cd,'ligand_p_centroid_eV':lp,'pd_centroid_sep_eV':abs(cd-lp),'nCuD_channels':ncu,'nCp_channels':nc,'nNp_channels':nn}
