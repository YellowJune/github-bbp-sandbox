...
def build(shorten_A):
    ...
    for ix in range(2):
        for j,(s,p) in enumerate(BASE):
            sy.append(s);fr.append(list(((p[0]+ix)/2,p[1],p[2])));tags.append((ix,j))
    itl=tags.index((0,3)); inn=tags.index((0,4))
    sy[itl]='Tl'; sy[inn]='N'
    # QE forces in the unrelaxed candidate point Tl(-z), N(+z), which shortens
    # their minimum-image separation. Apply that direction symmetrically.
    dz=shorten_A/(2*C0)
    fr[itl][2]=(fr[itl][2]-dz)%1.0
    fr[inn][2]=(fr[inn][2]+dz)%1.0
...
def metrics(prefix,ef):
    ...
    return {'cu_d_EFwin':float(trap(cu[mask],E[mask])),'ligand_p_EFwin':float(trap(lig[mask],E[mask])),'hybrid_overlap':float(trap(np.sqrt(np.clip(cu[mask],0,None)*np.clip(lig[mask],0,None)),E[mask])),'pd_centroid_sep_eV':abs(cent(cu)-cent(lig))}

def parse_forces(text):
    F=[]
    conv=13.605693122994/0.529177210903
    for m in re.finditer(r'atom\s+(\d+)\s+type\s+\d+\s+force\s+=\s+([-0-9.Ee+]+)\s+([-0-9.Ee+]+)\s+([-0-9.Ee+]+)',text,re.I):
        F.append(np.array([float(m.group(i)) for i in (2,3,4)])*conv)
    return float(max(np.linalg.norm(x) for x in F)) if F else np.nan

def main():
    ap=argparse.ArgumentParser();ap.add_argument('--shorten',type=float,required=True);ap.add_argument('--ppdir',required=True);ap.add_argument('--meta',required=True);ap.add_argument('--output',required=True);ap.add_argument('--scratch',required=True);ap.add_argument('--pw',default='/opt/espresso/7.5/pw.x');ap.add_argument('--proj',default='/opt/espresso/7.5/projwfc.x');a=ap.parse_args()
    out=Path(a.output);out.mkdir(parents=True,exist_ok=True);scratch=Path(a.scratch);scratch.mkdir(parents=True,exist_ok=True)
    sy,fr,cell=build(a.shorten);pm=pmap(a.ppdir,a.meta,sorted(set(sy)));pref='loc'+str(int(round(a.shorten*100)))
    inp=out/'scf.in';oo=out/'scf.out';write_scf(inp,pref,sy,fr,cell,pm,a.ppdir,str(scratch));rc=run(a.pw,inp,oo);txt=oo.read_text(errors='ignore') if oo.exists() else ''
    def last(p):
        m=re.findall(p,txt,re.I);return float(m[-1]) if m else np.nan
    rec={'shorten_A':a.shorten,'initial_TlN_A':C0*0.205029-a.shorten,'scf_rc':rc,'converged':'convergence has been achieved' in txt,'fermi_eV':last(r'the Fermi energy is\s+([-0-9.Ee+]+)'),'total_magnetization_Bohr':last(r'total magnetization\s+=\s+([-0-9.Ee+]+)\s+Bohr'),'max_force_eV_A':parse_forces(txt),'pressure_kbar':last(r'P=\s*([-0-9.Ee+]+)')}
    if rc==0 and np.isfinite(rec['fermi_eV']):
        ...
        if prc==0:rec.update(metrics(fil,rec['fermi_eV']))
