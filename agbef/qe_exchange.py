...
def write_scf(path,prefix,s,labels,pm,ppdir,outdir,U,mode):
    ...
    L=['&CONTROL'," calculation='scf',",f" prefix='{prefix}',",f" pseudo_dir='{ppdir}',",f" outdir='{outdir}',"," tstress=.true., tprnfor=.true., disk_io='low',",'/','&SYSTEM',' ibrav=0,',f' nat={len(s)}, ntyp={len(sp)},',' ecutwfc=55.0, ecutrho=440.0,'," occupations='smearing', smearing='mv', degauss=.012,",' nspin=2,']
    ...
    L+=['ATOMIC_SPECIES']+[f' {x} {MASS[x]:.8f} {pm[x]}' for x in sp]+['ATOMIC_POSITIONS crystal']
