...
MASS={'Sc':44.95591,'Ti':47.867,'CuA':63.546,'CuB':63.546,'N':14.007}
BASE={'Sc':'Sc','Ti':'Ti','CuA':'Cu','CuB':'Cu','N':'N'}
...
def site_labels(s):
    ...
    for site in s:
        ...
        if e!='Cu': labels.append(e); continue
        fx,fy,_=site.frac_coords
        ix=int(round((fx%1)*3))%3; iy=int(round((fy%1)*2))%2
        labels.append('CuA' if (ix+iy)%2==0 else 'CuB')
    return labels


def write_scf(path,prefix,s,labels,pm,ppdir,outdir,U):
    sp=[]
    for x in labels:
        if x not in sp: sp.append(x)
    a,b,c=s.lattice.abc
    k=(max(2,min(4,round(18/a))),max(2,min(5,round(18/b))),max(5,min(10,round(24/c))))
    ...
    for i,x in enumerate(sp,1):
        mag=.58 if x=='CuA' else (-.58 if x=='CuB' else (.03 if x=='N' else 0.0))
        L.append(f' starting_magnetization({i})={mag:.3f},')
    L+=['/','&ELECTRONS',' conv_thr=1.d-8, mixing_beta=.20, electron_maxstep=360,','/']
    if U>0:
        L+=['HUBBARD (ortho-atomic)',f' U CuA-3d {U:.6f}',f' U CuB-3d {U:.6f}']
...
def sum_orbitals(prefix,labels,kind):
    E=None; y=None; count=0
    for lab in labels:
        fs=glob.glob(prefix+f'.pdos_atm#*({lab})_wfc#*({kind})')
        for f in fs:
...
def metrics(prefix,ef):
    E,cu,ncu=sum_orbitals(prefix,['CuA','CuB'],'d')
    EN,n,nn=sum_orbitals(prefix,['N'],'p')
    Et,td=total_dos(prefix)
    if E is None or n is None or not np.isfinite(ef): return {}
    tr=np.trapezoid
    def integ(y,lo,hi):
        m=(E>=ef+lo)&(E<=ef+hi); return float(tr(y[m],E[m])) if m.sum()>=2 else np.nan
    mc=(E>=ef-4.0)&(E<=ef+1.0)
    def centroid(y):
        den=tr(y[mc],E[mc]); return float(tr(E[mc]*y[mc],E[mc])/den) if abs(den)>1e-12 else np.nan
    cd,lp=centroid(cu),centroid(n)
    rec={
        'cu_d_pm0p10':integ(cu,-.10,.10),'n_p_pm0p10':integ(n,-.10,.10),
        'cu_d_pm0p25':integ(cu,-.25,.25),'n_p_pm0p25':integ(n,-.25,.25),
        'cu_d_pm0p50':integ(cu,-.50,.50),'n_p_pm0p50':integ(n,-.50,.50),
        'pd_hybrid_pm0p25':integ(np.sqrt(np.clip(cu,0,None)*np.clip(n,0,None)),-.25,.25),
        'pd_hybrid_pm0p50':integ(np.sqrt(np.clip(cu,0,None)*np.clip(n,0,None)),-.50,.50),
        'cu_d_centroid_eV':cd,'n_p_centroid_eV':lp,'pd_centroid_sep_eV':abs(cd-lp),
        'nCuD_channels':ncu,'nNp_channels':nn,
    }
    if Et is not None and len(Et)>3:
        m=(Et>=ef-.10)&(Et<=ef+.10)
        rec['total_DOS_pm0p10']=float(np.trapezoid(td[m],Et[m])) if m.sum()>=2 else np.nan
        rec['DOS_at_EF_interp']=float(np.interp(ef,Et,td))
    return rec
...
def main():
    ...
    s=Structure.from_file(a.cif); labels=site_labels(s); out=Path(a.out);out.mkdir(parents=True,exist_ok=True);scratch=Path(a.scratch);scratch.mkdir(parents=True,exist_ok=True)
    pm=pmap(a.ppdir,a.meta,sorted(set(labels))); rows=[]
    for U in [0.,6.]:
        tag=f'U{int(U)}';od=scratch/tag;od.mkdir(exist_ok=True);pref=f'scti5_{int(a.pressure_atm)}atm_{tag}'
        inp=out/f'scf_{tag}.in';oo=out/f'scf_{tag}.out';write_scf(inp,pref,s,labels,pm,a.ppdir,str(od),U)
