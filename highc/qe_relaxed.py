...
MASS={'Nb':92.90637,'W':183.84,'CuA':63.546,'CuB':63.546,'C':12.011,'N':14.007}
BASE={'Nb':'Nb','W':'W','CuA':'Cu','CuB':'Cu','C':'C','N':'N'}
...
def write_scf(path,prefix,s,labs,pm,ppdir,outdir,U):
    ...
    L=['&CONTROL'," calculation='scf',",f" prefix='{prefix}',",f" pseudo_dir='{ppdir}',",f" outdir='{outdir}',"," tstress=.true., tprnfor=.true., disk_io='low',",'/','&SYSTEM',' ibrav=0,',f' nat={len(s)}, ntyp={len(sp)},',' ecutwfc=65.0, ecutrho=520.0,'," occupations='smearing', smearing='mv', degauss=.018,",' nspin=2,']
    for i,x in enumerate(sp,1):
        mag=.58 if x=='CuA' else (-.58 if x=='CuB' else (.03 if x in {'C','N'} else 0.0));L.append(f' starting_magnetization({i})={mag:.3f},')
    L+=['/','&ELECTRONS',' conv_thr=1.d-8, mixing_beta=.16, electron_maxstep=450,','/']
...
def metrics(prefix,ef):
    ...
    cd,cp,np_=cent(cu),cent(c) if c is not None else np.nan,cent(n) if n is not None else np.nan
    return {'cu_d_pm0p10':integ(cu,-.10,.10),'C_p_pm0p10':integ(c,-.10,.10) if c is not None else np.nan,'N_p_pm0p10':integ(n,-.10,.10) if n is not None else np.nan,'cu_d_pm0p25':integ(cu,-.25,.25),'ligand_p_pm0p25':integ(lig,-.25,.25),'pd_hybrid_pm0p25':integ(np.sqrt(np.clip(cu,0,None)*np.clip(lig,0,None)),-.25,.25),'pd_hybrid_pm0p50':integ(np.sqrt(np.clip(cu,0,None)*np.clip(lig,0,None)),-.50,.50),'cu_d_centroid_eV':cd,'C_p_centroid_eV':cp,'N_p_centroid_eV':np_,'nCuD_channels':ncu,'nCp_channels':nc,'nNp_channels':nn}

def main():
    ap=argparse.ArgumentParser();ap.add_argument('--cif',required=True);ap.add_argument('--pressure-atm',type=float,required=True);ap.add_argument('--name',required=True);ap.add_argument('--ppdir',required=True);ap.add_argument('--meta',required=True);ap.add_argument('--scratch',required=True);ap.add_argument('--out',required=True);ap.add_argument('--pw',default='/opt/espresso/7.5/pw.x');ap.add_argument('--proj',default='/opt/espresso/7.5/projwfc.x');a=ap.parse_args()
    ...
    for U in [0.,6.]:
        tag=f'U{int(U)}';od=scratch/tag;od.mkdir(exist_ok=True);pref=f'highc_{int(a.pressure_atm)}_{tag}'
        inp=out/f'scf_{tag}.in';oo=out/f'scf_{tag}.out';write_scf(inp,pref,s,labs,pm,a.ppdir,str(od),U)
        rc=run(a.pw,str(inp),str(oo));rec={'candidate':a.name,'pressure_atm':a.pressure_atm,'U_Cu_eV':U,'scf_rc':rc,'natoms':len(s)}
        if rc==0:
