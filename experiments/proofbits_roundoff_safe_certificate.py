...
def collect_base(m,tok):
    ...
    hs=[]
    for t in texts:
        x=tok(t,return_tensors='pt',truncation=True,max_length=MAX_LEN); H=hidden(m,**x)[0].float().cpu(); hs.append(H[max(0,H.shape[0]-1-(len(hs)%5))])
    return torch.stack(hs)
def gamma(n):
    x=n*U32
...
def cert_upper(h,U,rowmax,D):
    # Conservative implementation-level upper bound for a FP32-FMA-style dense
    # reference. One gamma covers possible downward error in U; one covers
    # possible upward error in the dense exact dot product.
    g=gamma(2*D)
...
def eval_states(H,W16,lo,hi,rowmax):
    ...
    for s in range(0,len(H),BATCH):
        ...
        for n in range(len(h)):
            ri=raw[n].nonzero().squeeze(1); si=safe[n].nonzero().squeeze(1); raw_counts.append(len(ri)); safe_counts.append(len(si)); oks.append(int(si[exact[n,si].argmax()])==int(ref[n])); slack_stats.append(float(sl[n].median()))
...
def boundary_alpha(za,zb,ya,it=50):
    l,r=0.,1.
    for _ in range(it):
        a=(l+r)/2; y=int(((1-a)*za+a*zb).argmax()); l,r=(a,r) if y==ya else (l,a)
    return (l+r)/2
def eval_boundary(H,Z,W16,lo,hi,rowmax):
    ...
    for k,(i,j) in enumerate(pairs):
        ...
        U=upper_raw(h,lo,hi); Us,sl=cert_upper(h,U,rowmax,D); pilots=torch.topk(Us,k=PILOT_K,dim=1).indices; B=exact.gather(1,pilots).amax(1); si=(Us[0]>=B[0]).nonzero().squeeze(1); ref=int(exact[0].argmax()); pred=int(si[exact[0,si].argmax()]); counts.append(len(si)); oks.append(pred==ref)
        tmp=exact[0].clone(); tmp[ref]=-torch.inf; ru=int(tmp.argmax()); margins.append(float(exact[0,ref]-exact[0,ru])); slacks.append(float(sl[0].median())); log(f'boundary {k+1}/{len(pairs)} cand={len(si)} margin={margins[-1]:.3e}')
...
def main():
    ...
    report={'kind':'proofbits_fp16_roundoff_safe_certificate','model':MODEL,'vocab':V,'hidden_dim':D,'pilot_k':PILOT_K,'fp32_unit_roundoff':U32,'gamma_2d':gamma(2*D),'certificate':'Uhat + 2*gamma_{2d}*||h||_1*M_i, M_i=max_j max(|lo_ij|,|hi_ij|)','natural':nat,'boundary':bnd,'caveat':'Conservative Higham-style rounding envelope for the stated FP32-FMA-style reference. A final GPU kernel must match or conservatively dominate its actual reduction/compute semantics.'}; OUT.write_text(json.dumps(report,indent=2)); print(json.dumps(report,indent=2))
